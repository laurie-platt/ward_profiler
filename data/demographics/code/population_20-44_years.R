# Demographics: Percentage of population aged 20-44 years, 2017 #
...
df <- read_xls("SAPE20DT8-mid-2017-ward-2017-syoa-estimates-unformatted.xls",
                 sheet = 4, skip = 3) %>%
  ...
  mutate(period = as.Date("2017-06-30", format = '%Y-%m-%d'),
         ...
         measure = "Percentage",
         unit = "Persons",
         value = round(rowSums(select(., `20`:`44`)/`All Ages`)*100, 1)) %>%
